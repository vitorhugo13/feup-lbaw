...
As stated above, before implementing mechanisms that ensure consistency and reliability in our database, an analysis had to be conducted in order to better assess what is of biggest priority. To make that analysis our workload contains:

 * The most important queries (SELECT) and their frequency
 * The most important modifications (INSERT, UPDATE, DELETE) and how often they are invoked
 * The expected performance of all queries and updates
 * An estimate of how many tuples each relation will have
...
| ---------------------- | ------------------ | ------------------------ | ----------------------------- |
| R01                    | content            | millions                 | hundreds of thousands per day |
| R02                    | report file        | thousands                | hundreds per day              |
| R02                    | report             | thousands                | hundreds per day              |
| R03                    | contest            | hundreds                 | dozens per day                |
| R04                    | post               | hundreds of thousands    | thousands per day             |
| R05                    | comment            | millions                 | hundreds of thousands per day |
| R06                    | thread             | hundred thousands        | thousands per day             |
| R07                    | reply              | millions                 | hundred of thousands per day  |
| R08                    | user               | millions                 | thousands per day             |
| R09                    | notification       | hundreds of thousands    | thousands per day             |
| R10                    | user_notification  | hundreds of thousands    | thousands per day             |
| R11                    | category           | dozens                   | units per month               |
| R12                    | post_category      | hundreds of thousands    | thousands per day             |
| R13                    | category_glory     | millions                 | thousands per day             |
| R14                    | star_post          | millions                 | dozens of thousands per day   |
| R15                    | star_category      | millions                 | thousands per day             |
| R16                    | rating             | hundreds of millions     | hundreds of thousands per day |
...
```sql
INSERT INTO "user"(username, email,password, release_date) VALUES($username, $email, $password, NULL);
```                                     
...
```sql
INSERT INTO comment(id) VALUES ($id);
```
...
```sql 
INSERT INTO rating(user_id, content_id, rating) VALUES($user_id, $content_id, $rating);
```
...
```sql
INSERT INTO report(report_file, author, reason) VALUES($report_file, $author, $reason);
```

| **Query**       | INSERT06                               |
| --------------- | -------------------------------------- |
| **Description** | Star a post                            |
| **Frequency**   | dozens of thousands per day            |
| **SQL code**    |   ↓           See below            ↓   |

```sql
INSERT INTO star_post(user_id, post_id) VALUES($user_id, $post_id);
```

| **Query**       | UPDATE01                               |
| --------------- | -------------------------------------- |
| **Description** |  Update user information               |
| **Frequency**   | dozens of thousand per day             |
| **SQL code**    |   ↓           See below            ↓   |
...
| **Query**       | UPDATE02                               |
| --------------- | -------------------------------------- |
| **Description** | Update the reviewer of a report file   |
| **Frequency**   | dozens per day                         |
| **SQL code**    |   ↓           See below            ↓   |

```sql
UPDATE report_file SET reviewer = $reviewer WHERE id = $id;
```
...
```sql
DELETE FROM star_post WHERE user_id = $user_id AND category_id = $category_id;
```

## 2. Proposed Indices

In some situations, it's important to retrieve certain rows quickly, so we choose some queries were it seemed useful to have indices in order to have this enhancement to the performance of our database and ultimately make the user experience more enjoyable.
...
| **Cardinality**     | high                            |
| **Clustering**      | No                              |
| **Justification**   | For trending filters, items are selected based on recent ratings, and therefore their time is accessed frequently. B-tree is used in order to do range searches. Cardinality is high since there are a lot of ratings and filtering will be common.|
| **SQL code**        |   ↓        See below        ↓   |
...
| **Cardinality**     | medium             				|
| **Clustering**      | Yes                             |
| **Justification**   | When viewing reports, they must be ordered by their date, therefore a trigger is useful specially using a B-tree structure, that allows quick sorting. Since the cardinality is medium, clustering can be applied and regularly updated.        |
| **SQL code**        |   ↓        See below        ↓   |
...
| **Index**           | IDX03           				|
| ------------------- | ------------------------------- |
| **Related queries** | Query for fresh posts (ND)      |
| **Relation**        | content            				|
| **Attribute**       | creation_time     				|
| **Type**            | B-Tree            				|
| **Cardinality**     | high             				|
| **Clustering**      | No                              |
| **Justification**   | For the fresh filter, content must be selected and ordered by their upload time. Since sorting is common a B-tree structure is best used. Given that the cardinality is high (filter will be used often) clustering is not appropriate in this case.      |
| **SQL code**        |   ↓        See below        ↓   |
...
| **Relation**        | category_glory     				|
| **Attribute**       | glory              				|
| **Type**            | B-Tree            				|
| **Cardinality**     | high             				|
| **Clustering**      | Yes                             |
| **Justification**   | Every time a user's page is viewed the top three categories with the most glory are displayed so their access should be quick. Given that we must sort based on their glory, a B-tree method is best. Although the cardinality is high, the information is not sensitive, so it doesn't have to be updated every time, therefore clustering can be used and only updated regulary.                         |
| **SQL code**        |   ↓        See below        ↓   |
...
| **Type**            | GiST                            |
| **Clustering**      | No                              |
| **Justification**   | When searching for posts, a full text search must be performed on the title of every post to increase performance an index is rather helpful. The title is dynamic data and therefore GiST should be used instead of GIN.                          |
| **SQL code**        |   ↓        See below        ↓   |
...
| **Type**            | GiST                            |
| **Clustering**      | No                              |
| **Justification**   | When searching for post of a particular user, a full text search must be performed on the username for each user which means using an index would greatly improve efficiency. Given that the username is dynamic data the appropriate structure is GiST.|
| **SQL code**        |   ↓        See below        ↓   |
...
| **Type**            | GiST                            |
| **Clustering**      | No                              |
| **Justification**   | When searching for posts of a given category a full text search is used to match the name to all of the existing category titles. Much like the previous cases, the categories associated with each post will vary constantly, therefore GiST is better suited in this case.|
| **SQL code**        |   ↓        See below        ↓   |
...
| **Trigger**         | TRIGGER01                       |
| ------------------- | ------------------------------- |
| **Description**     | Increase the number of posts in a category                                                |
| **Justification**   | This trigger is used so that the number of posts in a category is increased immediately whenever a new post is created, that is, for each created post, the categories associated with that post are checked and in each of them the number of posts in that category is incremented by one.                         |
| **SQL code**        |   ↓        See below        ↓   |
...
| **Trigger**         | TRIGGER02                       |
| ------------------- | ------------------------------- |
| **Description**     | Decrease the number of posts in a category                                                |
| **Justification**   | This trigger is used so that the number of posts in a category is decremented immediately whenever a post is deleted, that is, for each deleted post, the categories associated with that post are checked and in each of them the number of posts in that category is decremented by one.                         |
| **SQL code**        |   ↓        See below        ↓   |
...
| **Trigger**         | TRIGGER03                       |
| ------------------- | ------------------------------- |
| **Description**     | Update the number of posts between two categories.|
| **Justification**   | When the user moves a post, then it has to be removed from a category and inserted into the other, which actually happens has a update internally. This trigger will be used to keep the number of posts correct when this happens. |
| **SQL code**        |   ↓        See below        ↓   |
...
| **Trigger**         | TRIGGER04                       |
| ------------------- | ------------------------------- |
| **Description**     | Blocked user can not add or update rating to content.                               |
| **Justification**   | Trigger that prevents a blocked user from rating, raising an exception every time it happens.                                                |
| **SQL code**        |   ↓        See below        ↓   |
...
| **Trigger**         | TRIGGER05                       |
| ------------------- | ------------------------------- |
| **Description**     | Blocked user can not remove a rating.                                                 |
| **Justification**   | Trigger that prevents a blocked user from removing a rating. If that type of user tries to perform such action an exception is raised.          |
| **SQL code**        |   ↓        See below        ↓   |
...
| **Trigger**         | TRIGGER06                       |
| ------------------- | ------------------------------- |
| **Description**     |  Add rating to a content        |
| **Justification**   |  Trigger used when someone rates a content. When a content receives a rating, either upvote or downvote, the number of upvotes/downvotes of that content will increase by one unit. Following the same process, if the rating happens in a post, the glory of the creator of the post will be increased or decreased as well as the user's glory for that category.|
| **SQL code**        |   ↓        See below        ↓   |
...
| **Trigger**         | TRIGGER07                       |
| ------------------- | ------------------------------- |
| **Description**     |  Remove rating from a content   |
| **Justification**   |  Trigger used when a someone removes rating from a content. When rating is taken from a content, either upvote or downvote, the number of upvotes/downvotes of a content will decrement by one unit. Following the same process,  if the rating happens in a post, the glory of the creator of the post will be increased or decreased as well as the user's glory for that category.|
| **SQL code**        |   ↓        See below        ↓   |
...
| **Trigger**         | TRIGGER08                       |
| ------------------- | ------------------------------- |
| **Description**     | Update rating to a content      |
| **Justification**   | Trigger used when someone updates their rating on a post. When a rating is updated, whether from upvote to downvote or vice versa, the number of upvotes/downvotes of a content will increased/decrement by one unit. Following the same process,  if the rating happens in a post, the glory of the creator of the post will be increased or decreased as well as the user's glory for that category.                                               |
| **SQL code**        |   ↓        See below        ↓   |
...
| **Trigger**         | TRIGGER09                       |
| ------------------- | ------------------------------- |
| **Description**     | Create category-user association|
| **Justification**   | Trigger to create a row in the table category_glory when a user assigns a new category to a post. In order to display the user's top categories in his/hers profile, we keep track of his/hers glory in each category. The three above triggers update that information while this one associates the user with a category on the first time the user posts something in that category. |
| **SQL code**        |   ↓        See below        ↓   |
...
| **Trigger**         | TRIGGER10                       |
| ------------------- | ------------------------------- |
| **Description**     | Increase the number of comments in a post                                                |
| **Justification**   | This trigger is used so that the number of comments in a post is increased everytime a new reply is inserted into a thread, that is, for each created comment, when it is added to a thread as a reply then the number of comments on the post is incremented by one.                         |
| **SQL code**        |   ↓        See below        ↓   |
...
| **Trigger**         | TRIGGER11                       |
| ------------------- | ------------------------------- |
| **Description**     | Decreases the number of comments in a post                                                |
| **Justification**   | This trigger is used so that the number of comments in a post is decreased everytime a reply is deleted from a thread, that is, for each deleted comment, it is removed from a thread and the number of comments of the corresponding post is decreased by one.                         |
| **SQL code**        |   ↓        See below        ↓   |
...
| **Trigger**         | TRIGGER12                       |
| ------------------- | ------------------------------- |
| **Description**     | Increases the number of comments in a post when a thread is added.                                               |
| **Justification**   | This trigger is used so that the number of comments in a post is increased everytime a new thread is inserted into a post, that is, for each created thread the number of comments on the post is incremented by one.                         |
| **SQL code**        |   ↓        See below        ↓   |
...
| ------------------- | ------------------------------- |
| **Description**     | Decreases the number of comments in a post                                                |
| **Justification**   | This trigger is used so that the number of comments in a post is decreased everytime a thread is deleted, that is, for each deleted thread the number of comments is decreased by the number of replies of the thread plus one (the main comment of the thread) .                         |
| **SQL code**        |   ↓        See below        ↓   |
...
Here are presented some needed transactions to assure the integrity of the data.

| T01                 | Insert a new post               |
| ------------------- | ------------------------------- |
| **Justification**   | A transaction is used to guarantee data integrity in the presence of simultaneous accesses. We use the **Repeatable Read** isolation level since a concurrent transaction may insert a tuple in the *content* table, which would cause inconsistent data to be stored.                                              |
...
| T02                 | Insert a new comment to a post  |
| ------------------- | ------------------------------- |
| **Justification**   | A transaction is used to guarantee data integrity in the presence of simultaneous accesses. We use the **Repeatable Read** isolation level since a concurrent transaction may insert a tuple in the *content* or the *comment* table, which would cause inconsistent data to be stored.                                              |
| **Isolation level** | REPEATABLE READ                 |
| **SQL code**        |   ↓        See below        ↓   |
...
| T03                 | Insert a reply to a comment     |
| ------------------- | ------------------------------- |
| **Justification**   | A transaction is used to guarantee data integrity in the presence of simultaneous accesses. We use the **Repeatable Read** isolation level since a concurrent transaction may insert a tuple in the *content* table which would cause inconsistent data to be stored in the *comment* table, or in the *comment* table, causing inconsistencies in the *reply* table.                                          |
| **Isolation level** | REPEATABLE READ                 |
| **SQL code**        |   ↓        See below        ↓   |
...
| ------------------- | ------------------------------- |
| **Justification**   | If a search result happens to be deleted by another transaction that is running concurrently, then the user might be presented with *Phantom Content* or errors might occur while trying to retrieve the data for a deleted post. The isolation level is **SERIALIZABLE READ ONLY** because only Selects are used in this transaction.                                            |
...
**09/04/2020**
* Fixed small typo on UPDATE03 and DELETE01
* Deleted triggers 3, 4 and 5
* Fixed trigger numerations
* Added triggers 10, 11, 12 ,13

***
GROUP2031, 29/03/2020
